...
import type { Memory, MemoryStats, Scope, SearchScope, Category } from "./types.js";
...
export function getMemoryById(db: DatabaseSync, id: number): Memory | undefined {
  return db.prepare("SELECT * FROM memories WHERE id = ?").get(id) as unknown as
    | Memory
    | undefined;
}
...
export function getStats(db: DatabaseSync, scope: SearchScope, cwd: string): MemoryStats[] {
  ...
  return db
    .prepare(
      `SELECT category, COUNT(*) as count FROM memories WHERE ${sf.where} GROUP BY category ORDER BY count DESC`
    )
...
export function getTotalCount(db: DatabaseSync, scope: SearchScope, cwd: string): number {
  const sf = scopeFilter(scope, cwd);
  const row = db
    .prepare(`SELECT COUNT(*) as total FROM memories WHERE ${sf.where}`)
    .get(...sf.params) as unknown as { total: number };
  return row.total;
}
...
export function getOverviews(db: DatabaseSync, cwd: string): { project: Memory[]; user: Memory[] } {
  const projectOverviews = db
    .prepare("SELECT * FROM memories WHERE category = 'overview' AND scope = 'project' AND project = ? ORDER BY updated_at DESC")
