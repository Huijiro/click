...
import { CategorySchema, OptionalSearchScope, type SearchScope, type Memory } from "../types.js";
...
export function registerRecallTool(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    parameters: Type.Object({
      query: Type.String({
        description:
          "Search query — keywords or phrases to match against memory titles and content",
      }),
      ...
      limit: Type.Optional(
        Type.Number({ description: "Max results to return (default: 20)", default: 20 })
      ),
    }),
